//! Offline parser tests for the `CampSaver` clean-HTTP scraper.
//!
...
use gear_nest_pipeline::scrapers::campsaver::{parse_listing_urls, parse_price, parse_product};
...
fn parses_core_product_fields() {
    let p = parse_product(PRODUCT_HTML, PRODUCT_URL).expect("parse product");
...
fn fetch_price_reads_offer() {
    let update = parse_price(PRODUCT_HTML, "NEM-DAGGER-OSMO-2P").expect("parse price");
...
fn missing_product_json_ld_is_an_error() {
    ...
    assert!(parse_product(html, "https://www.campsaver.com/x.html").is_err());
}
